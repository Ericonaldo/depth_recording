...
def delete_files(directory, idx=None, min_idx=None, max_idx=None):
    # Validate input
    if (min_idx is not None and max_idx is None) or (min_idx is None and max_idx is not None):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Delete files based on index or range of indices.")
    ...
    parser.add_argument('--min_idx', type=int, help="Minimum index for range of files to delete.")
    parser.add_argument('--max_idx', type=int, help="Maximum index for range of files to delete.")
